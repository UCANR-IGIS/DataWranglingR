...
```{r chunk09}
## Using the anonymized research data table and personal info table, create a
## tidy (i.e., long) data frame of just the scores of the senior students.

ss_seniors_tbl <- ss_anon_tbl |> 
  left_join(ss_pii_tbl |> select(year, student_id), 
            by = "student_id") |> 
  filter(year == "SNR") |> 
  drop_na(score)

ss_seniors_tbl |> slice(1:20)
```
...
```{r chunk13}
## Compute the number of majors each student and save it as a new column

ss_nummajors_tbl <- ss_majors_tbl |> 
  group_by(student_id) |> 
  summarize(num_majors = n())

head(ss_nummajors_tbl)

ss_pii_tbl |> 
  left_join(ss_nummajors_tbl, by = "student_id") |> 
  select(name, student_id, major, num_majors)
```
...
```{r chunk17}
## Flip the rows and columns in the previous table, so that the rows represent
## majors, and the columns represent the 4 discussion sections.
...
ss_dates_tbl |> 
  ...
  summarize(score_avg = mean(score, na.rm = TRUE))
```
